"""Plot the v3 full-sweep matrix: N ∈ {10K, 100K, 1M} × sel ∈ {1%, 0.1%}.

v3 vs v2:
  - Baseline fix: filter-aware boost is opt-in (gated by `adaptive_search_nprobe`),
    size_based gradient {24, 32, 64} at {500K, 1M, >1M} is always on.
  - Unified bench harness: ONE writer (both bloom+synopsis enabled), shared
    across all 8 cube cells. Cross-cell comparisons are now apples-to-apples.

Reads CSVs from this directory. Produces:

  hypercube_<N>_<sel>.png     — 4-panel summary, 6 of these.
  recall_vs_io.png            — recall vs heads_fetched, all 6 workloads.
  scaling_with_N.png          — recall, drop_ratio, latency vs N (rows: sel).
  selectivity_compare.png     — sel=1% vs sel=0.1% side-by-side per cell.
  storage_cost.png            — unified-blob bytes + build time across N.
  audit_table.png             — bad_drops heatmap; must be all-zero.
"""
...
def recall_vs_io(out_path: Path):
    ...
    ax.set_ylabel("recall@10 (mean)")
    ax.set_title("v3: Recall vs I/O across the full N × selectivity matrix\n"
...
def scaling_with_N(out_path: Path):
    ...
    for r, sel in enumerate(sels):
        ...
        ax.set_title(f"v3: Recall vs N — sel={sel*100:g}%")
        ax.set_ylim(0, 1.05)
        ax.grid(True, alpha=0.3)
        if r == 0:
        ...
        ax.set_xlabel("N (records)")
        ax.set_ylabel("drop_ratio")
        ax.set_title(f"v3: Gate effectiveness — sel={sel*100:g}%")
        ax.set_ylim(0, 1.05)
        ax.grid(True, alpha=0.3)
        # heads_fetched
        ax = axes[r, 2]
        ...
        ax.set_title(f"v3: I/O cost — sel={sel*100:g}%")
        ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(out_path, dpi=130, bbox_inches="tight")
    plt.close()
...
def selectivity_compare(out_path: Path):
    ...
    for i, n in enumerate(sizes):
        ...
        ax.set_ylabel("recall@10")
        ax.set_title(f"v3: N={n//1000}K — recall by cell × selectivity")
        ax.set_ylim(0, 1.05)
        ax.grid(True, alpha=0.3, axis="y")
        ax.legend(fontsize=8)
        ...
        ax.set_xticklabels([CELL_LABELS[c][0] for c in CELL_ORDER], rotation=30, ha="right")
        ax.set_ylabel("heads_fetched")
        ax.set_title(f"v3: N={n//1000}K — I/O by cell × selectivity")
        ax.grid(True, alpha=0.3, axis="y")
        ax.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(out_path, dpi=130, bbox_inches="tight")
...
def storage_cost(out_path: Path):
    """v3 unified-bench: one entry `unified` per (N, sel) summary file.

    Plot blob storage and build time across N. The unified blob holds
    both bloom + synopsis; we don't split them per-flavor anymore.
    """
    summaries = []
    ...
    # Aggregate to (n_records) → max blob, mean build_ms across selectivities.
    grouped = (
        sdf.groupby(["n_records"])
        .agg(blob_bytes=("blob_bytes", "max"), build_ms=("build_ms", "mean"))
        .reset_index()
        .sort_values("n_records")
    )
    n_values = grouped["n_records"].tolist()
    ...
    ax.bar(
        x,
        [v / 1e6 for v in grouped["blob_bytes"]],
        color="#d62728",
        label="bloom + synopsis (combined)",
    )
    ax.set_xticks(x)
    ...
    ax.set_title("v3: Unified-bench gate blob storage by N\n(bloom + synopsis blobs combined)")
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3, axis="y")
    for i, v in enumerate(grouped["blob_bytes"]):
        ax.text(i, v / 1e6, f"{v/1e6:.1f} MB", ha="center", va="bottom", fontsize=8)

    ax = axes[1]
    ax.plot(
        grouped["n_records"],
        grouped["build_ms"] / 1000.0,
        marker="o",
        color="#1f77b4",
    )
    ax.set_xscale("log")
    ...
    ax.set_title("v3: Unified-bench index build time vs N")
    ax.grid(True, alpha=0.3)
    for n, t_ms in zip(grouped["n_records"], grouped["build_ms"]):
        ax.annotate(f"{t_ms/1000:.0f}s", (n, t_ms/1000.0),
...
def audit_check(out_path: Path):
    ...
    for n, b, sel, _, agg in all_workloads():
        for cell, r in agg.iterrows():
            rows.append(
                {
                    "label": (f"N={n//1000}K, sel={sel*100:.1f}%"
                              if n < 1_000_000
                              else f"N={n//1_000_000}M, sel={sel*100:.1f}%"),
                    "cell": cell,
    ...
    ax.set_title("v3: Gate-audit invariant — bad_drops per (workload, cell)")
    plt.tight_layout()
    plt.savefig(out_path, dpi=130, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    for n, b, sel, df, _ in workloads:
        ...
        title = (f"v3 hypercube — SIFT1M N={n_label}, sel={sel*100:g}%, "
                 f"unified-bench, MAX_FACTOR=16, size_based={{24,32,64}}")
        out = HERE / f"hypercube_n{n}_sel{int(round(1/sel))}.png"
        four_panel(df, title, out)
    recall_vs_io(HERE / "recall_vs_io.png")
